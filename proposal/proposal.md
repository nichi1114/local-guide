...
2. **Add and view points of interest**
   ...
   - **Technical Approach:** Use core components like View, Text, Button, TextInput, etc. to manage POI creation/update. The category can be fetched from Google Places or selected from a dropdown picker, preferably a tag-based picker if fetching fails.
...
6. **Backend Integration:**
   - **Feature:** Fetch place categories from a public API (e.g., Google Places).
   - **Technical Approach:** Send a Google Places Nearby Search request using the location, radius, and API key, then process the response to extract the primary type of the place. A fallback category picker or text input may be used if fetching fails.
...
Sitao:
   - Build Home Screen to fetch and list user-specific POIs from Async Storage.
...
Sitao:
   ...
   - Store latitude & longitude locally.
